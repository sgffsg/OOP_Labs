...
#include "../Lab2_2/CalculatorController.h"
...
std::string GetAnsFromParser(const std::string& command, CalculatorController& calculatorController)
{
	std::istringstream input(command);
	std::stringstream out;
	calculatorController.ProcessInput(input, out);
	std::string ans;
	std::string line;
...
SCENARIO("Parsing commands")
{
	...
	{
		std::string ans = GetAnsFromParser("command x", calculatorController);
		CHECK(ans == "Unknown command\n");
	}
	WHEN("Invalid var command")
	{
		std::string ans = GetAnsFromParser("createvar x", calculatorController);
		CHECK(ans == "Invalid command\n");
	}
	WHEN("Invalid var command, invalid identifier")
	{
		std::string ans = GetAnsFromParser("var 1x", calculatorController);
		CHECK(ans == "Invalid command\n");
	}
	WHEN("Var command")
	{
		std::string ans = GetAnsFromParser("var x", calculatorController);
		CHECK(ans.empty());
	}
	WHEN("Invalid let command")
	{
		std::string ans = GetAnsFromParser("let x", calculatorController);
		CHECK(ans == "Invalid command\n");
	}
	WHEN("Invalid let command, invalid identifier")
	{
		std::string ans = GetAnsFromParser("let 1x = 2", calculatorController);
		CHECK(ans == "Invalid command\n");
	}
	WHEN("Let command")
	{
		std::string ans = GetAnsFromParser("let x = 4", calculatorController);
		CHECK(ans.empty());
	}
	WHEN("Invalid fn command")
	{
		std::string ans = GetAnsFromParser("fn f", calculatorController);
		CHECK(ans == "Invalid command\n");
	}
	WHEN("Invalid fn command, invalid identifier")
	{
		std::string ans = GetAnsFromParser("let x=8\nfn 1f = x * x", calculatorController);
		CHECK(ans == "Invalid command\n");
	}
	WHEN("Fn command")
	{
		std::string ans = GetAnsFromParser("let x=8\nfn f = x * x", calculatorController);
		CHECK(ans.empty());
	}
	WHEN("Invalid print command")
	{
		std::string ans = GetAnsFromParser("print x, y", calculatorController);
		CHECK(ans == "Invalid command\n");
	}
	WHEN("Print command var value")
	{
		std::string ans = GetAnsFromParser("let x=4\nprint x", calculatorController);
		CHECK(ans == "4.00\n");
	}
	WHEN("Print command fn value")
	{
		std::string ans = GetAnsFromParser("let x=4\nfn f=x*x\nprint f", calculatorController);
		CHECK(ans == "16.00\n");
	}
	WHEN("Invalid printvars command")
	{
		std::string ans = GetAnsFromParser("let x=4\nlet y=5\nprintvarsss", calculatorController);
		CHECK(ans == "Invalid command\n");
	}
	WHEN("Printvars command")
	{
		std::string ans = GetAnsFromParser("let x=4\nlet y=5\nprintvars", calculatorController);
		CHECK(ans == "x:4.00\ny:5.00\n");
	}
	WHEN("Invalid printfns command")
	{
		std::string ans = GetAnsFromParser("let x=4\nlet y=5\nfn f1=x*y\nfn f2=x*x\ndsdprintfns", calculatorController);
		CHECK(ans == "Invalid command\n");
	}
	WHEN("Printfns command")
	{
		std::string ans = GetAnsFromParser("let x=4\nlet y=5\nfn f1=x*y\nfn f2=x*x\nprintfns", calculatorController);
		CHECK(ans == "f1:20.00\nf2:16.00\n");
	}
	WHEN("Using reserved identifier")
	{
		std::string ans = GetAnsFromParser("var let", calculatorController);
