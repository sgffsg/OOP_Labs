...
void CalculatorController::ProcessInput(std::istream& in, std::ostream& out)
{
	std::string line;
...
void CalculatorController::HandleCommand(const std::string& str, std::ostream& out)
{
...
void CalculatorController::ParseCommand(const std::regex& pattern, std::smatch& match, const std::string& command) const
{
...
void CalculatorController::VarCommand(const std::string& identifier)
{
	CheckIdentifierWithReservedWords(identifier);
	m_calculator.Var(identifier);
}

void CalculatorController::LetCommand(const std::string& identifier, const std::string& value)
{
	CheckIdentifierWithReservedWords(identifier);
	m_calculator.Let(identifier, value);
}

void CalculatorController::FnCommand(const std::string& identifier, const std::string& arg1, const std::string& operation, const std::string& arg2)
{
	CheckIdentifierWithReservedWords(identifier);
	if (arg2.empty())
	{
...
void CalculatorController::PrintCommand(const std::string& identifier, std::ostream& out) const
{
...
void CalculatorController::PrintVarsCommand(std::ostream& out) const
{
	out << std::fixed << std::setprecision(PRECISION);
...
void CalculatorController::PrintFnsCommand(std::ostream& out) const
{
	out << std::fixed << std::setprecision(PRECISION);
	auto fn = CreateFnToPrintValue(out);
...
void CalculatorController::CheckIdentifierWithReservedWords(const std::string& identifier) const
{
...
std::function<void(std::string, double)> CalculatorController::CreateFnToPrintValue(std::ostream& out) const
{
