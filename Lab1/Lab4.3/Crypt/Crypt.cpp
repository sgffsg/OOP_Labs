...
Args ParseArgs(int argc, char* argv[])
{
	...
	if (mode == "crypt")
	{
		args.mode = WorkMode::Crypt;
	}
	else if (mode == "decrypt")
	{
		args.mode = WorkMode::Decrypt;
	}
	else
	{
...
void CryptOrDecryptFile(const WorkMode& mode, const std::string& inFileName, const std::string& outFileName, int key)
{
	...
	if (mode == WorkMode::Crypt)
	{
		Crypt(inFile, outFile, key);
	}
	if (mode == WorkMode::Decrypt)
	{
